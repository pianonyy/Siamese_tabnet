...
class TabModel(BaseEstimator):
    ...
    def _train_batch(self, X, t, y):
        ...
        output0, M_loss = self.network(torch.cat((X, zeros), dim = 1) )
        output1, M_loss = self.network(torch.cat((X, ones), dim = 1) )

        
        m = nn.Softmax(dim=1)
        output0 = m(output0)
        output1 = m(output1)
        output = output1[:,1] - output0[:,1]
        ...
        uplift_pred_Y = t * output1[:,1] + (ones - t) * output0[:,1] 
        ...
        uplift_pred = output1[:,1] - output0[:,1] 
        
        
        # print("uplift_pred", uplift_pred_Y)
        # print("uplift_pred", y)
        sum_of_losses = torch.mean(  (1-alpha) * loss_MSE(Z_trans, uplift_pred) + alpha * loss_cross( uplift_pred_Y, y))
        # Perform backward pass and optimization
        sum_of_losses -= self.lambda_sparse * M_loss
        sum_of_losses.backward()
        if self.clip_value:
            clip_grad_norm_(self.network.parameters(), self.clip_value)
        self._optimizer.step()

        batch_logs["loss"] = sum_of_losses.cpu().detach().numpy().item()
    ...
    def _predict_epoch(self, name, loader):
        ...
        # Main loop
        for batch_idx, (X, t, y) in enumerate(loader):
    ...
    def _predict_batch(self, X):
        ...
        # scores_0, _ = self.network(torch.cat((X, zeros), dim = 1))
        # scores_1, _ = self.network(torch.cat((X, ones), dim = 1))

        # m = nn.Softmax(dim=1)
        # scores_0 = m(scores_0)
        # scores_1 = m(scores_1)

        # scores = scores_1[:,1] - scores_0[:,1] 
    ...
    def _construct_loaders(self, X_train, T_train, y_train, eval_set):
        ...
        for i, (X, t, y) in enumerate(eval_set):
            y_mapped = self.prepare_target(y)
            eval_set[i] = (X, t, y_mapped)
    ...
    def _compute_feature_importances(self, loader):
        ...
        for data, treat, targets in loader:
            data = data.to(self.device).float()
            M_explain, masks = self.network.forward_masks(data)
